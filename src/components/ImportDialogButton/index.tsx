...
const ImportDialogButton = (props: ImportDialogButtonProps) => {
    ...
    const typeName =  valueEnum[value]?.text || ''

    const columns = valueEnum[value]?.columns|| []
    ...
    const handleImportExcel = () => {
        window?._utils?.importExcel([], (res: any) => {
            const [{data}] = res
            console.log('res',res)
            const result = data.filter(v=>v).map((item:any) => {
